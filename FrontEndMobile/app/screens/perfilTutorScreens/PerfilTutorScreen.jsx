...
export default function ProfileScreen({ navigation }) {
  ...
  return (
    <ScrollView style={{ flex: 1, backgroundColor: COLORS.bg }} contentContainerStyle={{ paddingBottom: 40 }}>
      ...
      <Section title="Conta">
        <Item icon={<Ionicons name="person-outline" size={20} color={COLORS.brand} />}
              label="Editar perfil" onPress={() => navigation?.navigate?.("EditProfile")} />
        <Item icon={<MaterialIcons name="pets" size={20} color={COLORS.brand} />}
              label="Meus pets" onPress={() => navigation?.navigate?.("Pets")} />
        <Item icon={<Ionicons name="calendar-outline" size={20} color={COLORS.brand} />}
              label="Agendamentos" onPress={() => navigation?.navigate?.("Appointments")} />
        <Item icon={<Ionicons name="document-text-outline" size={20} color={COLORS.brand} />}
              label="Lembretes" onPress={() => navigation?.navigate?.("Reminders")} />
      </Section>

      <Section title="Preferências">
        <Item icon={<Ionicons name="notifications-outline" size={20} color={COLORS.brand} />}
              label="Notificações" onPress={() => navigation?.navigate?.("Notifications")} />
        <Item icon={<Ionicons name="shield-checkmark-outline" size={20} color={COLORS.brand} />}
              label="Privacidade" onPress={() => navigation?.navigate?.("Privacy")} />
        <Item icon={<Ionicons name="color-palette-outline" size={20} color={COLORS.brand} />}
              label="Tema" value="Padrão" onPress={() => {}} />
      </Section>

      <Section>
        <Item icon={<Ionicons name="help-circle-outline" size={20} color={COLORS.brand} />}
              label="Ajuda" onPress={() => navigation?.navigate?.("Help")} />
        <Item icon={<Ionicons name="information-circle-outline" size={20} color={COLORS.brand} />}
              label="Sobre" onPress={() => navigation?.navigate?.("About")} />
        <Item icon={<Ionicons name="log-out-outline" size={20} color={COLORS.danger} />}
              label="Sair" danger onPress={() => { /* TODO: signOut() */ }} />
      </Section>
    </ScrollView>
  );
}
...
const styles = StyleSheet.create({
  ...
  editBtn: {
    ...
    backgroundColor: "#ECEFF5", paddingHorizontal: 12, paddingVertical: 8, borderRadius: 10,
  },
  editText: { fontSize: 14, fontWeight: "600", color: COLORS.text },
  card: {
    backgroundColor: COLORS.card, marginHorizontal: 20, marginTop: 8, borderRadius: 16, padding: 16,
  ...
  itemIcon: {
    width: 32, height: 32, borderRadius: 8, backgroundColor: COLORS.tint,
