...
export default function LembretesList({ lembretes = [], onAtualizar }) {
  ...
  // Agrupamento de lembretes por hora
  ...
  const handleAvaliar = (lembrete) => {
    const translatedStatus = translateAppointmentStatus(lembrete.status, lembrete.isRated);
    if (lembrete.isRated || translatedStatus === "Avaliado") return;

    const serviceId = lembrete.service?._id || null;
    const providerId = lembrete.provider?._id || null;

    router.push({
      pathname: "/screens/lembreteScreens/AvaliarScreen",
      params: {
        appointmentId: lembrete.id,
        serviceId,
        providerId,
      },
    });
...
const styles = StyleSheet.create({
  ...
  horaPrincipal: { color: "#8E8E8E", fontSize: 13, fontWeight: "400", marginBottom: 10 },
  card: { backgroundColor: "#FFF", borderRadius: 14, flexDirection: "row", paddingVertical: 12, paddingHorizontal: 14, shadowColor: "#000", shadowOpacity: 0.05, shadowRadius: 6, elevation: 2, marginBottom: 10 },
  ...
  btnAvaliar: { backgroundColor: "#2F8B88", paddingVertical: 6, paddingHorizontal: 12, borderRadius: 8 },
  btnAvaliarText: { color: "#fff", fontSize: 13, fontWeight: "bold" },
  textAvaliado: { color: "#2F8B88", fontSize: 13, fontWeight: "600" },
});
